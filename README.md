...
- **Real image generation** — say *"generate an image of a cozy coffee shop at night"* and Halo calls an actual image model (Google Nano Banana via your existing OpenRouter key by default; OpenAI `gpt-image-1.5` if you add `OPENAI_API_KEY` and set `HALO_IMAGE_PROVIDER=openai`), saves the PNG to `~/Pictures/Halo`, and opens it. It's a **local action** — never dispatched to a coding agent (which would only draw an SVG).
