...
- **Real image generation** — say *"generate an image of a cozy coffee shop at night"* and Halo produces an actual PNG (to `~/Pictures/Halo`) and opens it. **Default: your Codex CLI's built-in `image_gen` tool** (`gpt-image-2`) — runs on your existing `codex login` (ChatGPT auth), **no separate API key**; needs ChatGPT Plus or higher. Fallbacks via `HALO_IMAGE_PROVIDER`: `openrouter` (Google Nano Banana via the OpenRouter key Halo already has) or `openai` (`gpt-image-1.5`, needs `OPENAI_API_KEY`). It's a **local action** — never dispatched to a coding agent as a prompt (which would only draw an SVG).
